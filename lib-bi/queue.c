...
{
    return deque_get_last(queue, out);
}
...
{
    return deque_remove_last(queue, out);
}
...
{
    return deque_add_first(queue, element);
}
...
{
    deque_zip_iter_init(&(iter->i), q1, q2);
}
