...
{
    return array_new_conf(conf, out);
}
...
{
    return array_add(stack, element);
}
...
{
    return array_get_last(stack, out);
}
...
{
    return array_remove_last(stack, out);
}
...
{
    array_zip_iter_init(&(iter->i), s1, s2);
}
